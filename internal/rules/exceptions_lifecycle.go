...
func (ExceptionsLifecycle) Evaluate(_ context.Context, input sdk.EvalContext, rule sdk.Rule) ([]sdk.Finding, error) {
	...
	for _, file := range files {
		...
		for _, item := range documents {
			...
			if !found {
				findings = append(findings, finding(rule, message+": collection "+spec.Collection+" is missing", file.Path, "collection", item.Line, item.Column))
				continue
			}
			entries, ok := collection.([]any)
			if !ok {
				return nil, fmt.Errorf("%s: collection %s must be a list", file.Path, spec.Collection)
			}
			seenIDs := make(map[string]bool)
			for index, raw := range entries {
				entry, ok := raw.(map[string]any)
				if !ok {
					findings = append(findings, finding(rule, message+": exception must be an object", file.Path, fmt.Sprintf("entry:%d", index), item.Line, item.Column))
					continue
				}
				...
				fields := map[string]string{
					"id":     spec.IDField,
					"reason": spec.ReasonField, "owner": spec.OwnerField, "ticket": spec.TicketField,
					"created": spec.CreatedField, "expires": spec.ExpiresField,
				}
				...
				if contains(spec.Required, "ticket") {
					ticket, parseTicketErr := url.Parse(fmt.Sprint(entry[spec.TicketField]))
					if parseTicketErr != nil || ticket.Scheme != "https" || ticket.Host == "" {
				...
				if createdErr == nil && expiresErr == nil {
					...
					if expires.Sub(created) > time.Duration(spec.MaximumDays)*24*time.Hour {
						problems = append(problems, fmt.Sprintf("lifetime exceeds %d days", spec.MaximumDays))
					}
					if input.Now.UTC().Truncate(24 * time.Hour).After(expires) {
						problems = append(problems, "exception is expired")
					}
				}
				if len(problems) > 0 {
					findings = append(findings, finding(rule, message+": "+strings.Join(problems, "; "), file.Path, key, item.Line, item.Column))
				}
			}
		}
	}
